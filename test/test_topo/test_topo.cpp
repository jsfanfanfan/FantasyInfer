...
TEST(test_ir, topo)
{
    ...
    std::string bin_path("/home/fjs/FantasyInfer/model_file/resnet18_batch1.pnnx.bin");
    std::string param_path("/home/fjs/FantasyInfer/model_file/resnet18_batch1.param");
...
{
    using namespace fantasy_infer;
    std::string bin_path("/home/fjs/FantasyInfer/model_file/simple_ops.pnnx.bin");
    std::string param_path("/home/fjs/FantasyInfer/model_file/simple_ops.pnnx.param");
    RuntimeGraph graph(param_path, bin_path);
    const bool init_success = graph.Init();
    ASSERT_EQ(init_success, true);
    graph.Build("pnnx_input_0", "pnnx_output_0");
...
{
    using namespace fantasy_infer;
    std::string bin_path("/home/fjs/FantasyInfer/model_file/simple_ops2.pnnx.bin");
    std::string param_path("/home/fjs/FantasyInfer/model_file/simple_ops2.pnnx.param");
    RuntimeGraph graph(param_path, bin_path);
    ASSERT_EQ(int(graph.graph_state()), -2);
    const bool init_success = graph.Init();
    ASSERT_EQ(init_success, true);
