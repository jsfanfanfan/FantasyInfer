    ...
    if (op->input_operands.empty()) {
      ...
      for (const auto &[_, input_operand] : input_operands_map) {
        ...
        if (!input_datas.empty()) {
          ...
          // 检查已存在的tensor形状是否正确，如果不正确则reshape
          for (int j = 0; j < batch; ++j) {
            sftensor input_tensor = input_datas.at(j);
            CHECK(input_tensor != nullptr) << "Input tensor is null";
            const std::vector<uint32_t> &tensor_shapes = input_tensor->shapes();
            
            if (input_operand_shape.size() == 4) {
              if (tensor_shapes.at(0) != (uint32_t)input_operand_shape.at(1) ||
                  tensor_shapes.at(1) != (uint32_t)input_operand_shape.at(2) ||
                  tensor_shapes.at(2) != (uint32_t)input_operand_shape.at(3)) {
                DLOG(WARNING)
                    << "The shape of tensor do not adapting with input operand";
                const auto &target_shapes = std::vector<uint32_t>{
                    (uint32_t) input_operand_shape.at(1), 
                    (uint32_t) input_operand_shape.at(2),
                    (uint32_t) input_operand_shape.at(3)};
                input_tensor->Reshape(target_shapes);
              }
            } else if (input_operand_shape.size() == 2) {
              if (tensor_shapes.at(0) != 1 ||
                  tensor_shapes.at(1) != (uint32_t)input_operand_shape.at(1) ||
                  tensor_shapes.at(2) != 1) {
                DLOG(WARNING)
                    << "The shape of tensor do not adapting with input operand";
                const auto &target_shapes =
                    std::vector<uint32_t>{(uint32_t) input_operand_shape.at(1)};
                input_tensor->Reshape(target_shapes);
              }
            } else {
              // current shape is 3
              if (tensor_shapes.at(0) != 1 ||
                  tensor_shapes.at(1) != (uint32_t)input_operand_shape.at(1) ||
                  tensor_shapes.at(2) != (uint32_t)input_operand_shape.at(2)) {
                DLOG(WARNING)
                    << "The shape of tensor do not adapting with input operand";
                const auto &target_shapes = std::vector<uint32_t>{
                    (uint32_t) input_operand_shape.at(1), 
          ...
          input_datas.clear();
          for (int j = 0; j < batch; ++j) {
            sftensor input_tensor;
            if (input_operand_shape.size() == 4) {
              input_tensor = TensorCreate(
                  (uint32_t)input_operand_shape.at(1), 
                  (uint32_t)input_operand_shape.at(2), 
                  (uint32_t)input_operand_shape.at(3));
            } else if (input_operand_shape.size() == 2) {
              input_tensor = TensorCreate((uint32_t)input_operand_shape.at(1));
            } else {
              // current shape is 3
              input_tensor = TensorCreate(
                  (uint32_t)input_operand_shape.at(1), 
            ...
            input_datas.push_back(input_tensor);
          }
        }
      }
    }
...
void RuntimeOperatorUtils::InitOperatorOutput(
  ...
  for (uint32_t i = 0; i < pnnx_operators.size(); ++i) {
    ...
    CHECK(operands.size() == 1) << "Only support one output in the fantasyInfer";
